...
bool BinarySearchTree<T>::search(const T& item) const {
   ...
   found = search(item, ptr);

   return found;
}

/**
 * Helper to recursively search a tree for an item
 *
 * Precondition: item is the item being searched for, node is the
 *      root of the tree being searched, and the location is the location
 *      that the item was found in.
 * Postcondition: Returns true if item is found, and false otherwise
 *
 * Worse-Case Time Complexity: O(h), where h is the heigh of the tree
 */
template <class T>
bool BinarySearchTree<T>::search(const T& item, Node * node) const {

   if (node == NULL) { // empty list
      return false;   
   }

   if (item < node->data) { // check left
      return search( item, node->left);
   }

   if (node->data < item) { // check right
...
void BinarySearchTree<T>::copyBinarySearchTree(Node * original, Node * &copy) {
   ...
   copy = new Node();
   if (original == NULL) { // empty tree
      copy = NULL;
   }
   else {
      Node * parent = NULL;
      copyBinarySearchTree(original,copy,parent);
   }

}

template <class T>
void BinarySearchTree<T>::copyBinarySearchTree(Node * original, Node * &copy, Node * &parent) {
   ...
   if (original->left != NULL) {
      ...
      copy->left = leftSide;
      copyBinarySearchTree(original->left, leftSide, copy);
   }
   if (original->right != NULL) {
      Node * rightSide = new Node();
      copy->right = rightSide;
